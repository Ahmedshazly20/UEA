...
export const EstektaaList: FC<{
  ...
  useEffect(() => {
    const fillData = async () => {
      ...
      var estList = await getLookupByType(LookupTypeEnum.Estektaa);
        if (estList != null) {
          setRuleList(estList);
         }
  ...
  const onCurrentPageChange = async (number: number) => {
    ...
    getDueList(searchEmp,searchRule,Number(AccrualSubtractRuleTypeEnum.Subtract),pageSize,number)
  };
  const onPageSizeChange = async (size: number) => {
    if (size !== pageSize) {
      setPageSize(size)
    }
   await getDueList(searchEmp,searchRule,Number(AccrualSubtractRuleTypeEnum.Subtract),size,pageNumber)
...
//   }, [searchEmp,searchRule]);
  //#region html
      const handleSearch = async() => {
        setLoading(true);
         await getDueList(searchEmp,searchRule,Number(AccrualSubtractRuleTypeEnum.Subtract),pageSize,pageNumber);
         setLoading(false);
        };
